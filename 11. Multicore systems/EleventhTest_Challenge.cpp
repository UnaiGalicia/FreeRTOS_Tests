...
void averageCalc(void * parameters){
    ...
    /*Important! AverageCalc runs on core 0. If we initialize the timer
    in setup, which runs in core 1, the timer will be pinned to core 1, and we
    want it in 0. So timer declarations go here:*/
...
void setup(){
    ...
    //Create both tasks
    xTaskCreatePinnedToCore(terminal, "Terminal task", 1024, NULL, 1, NULL, pro_cpu);
